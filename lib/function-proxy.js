...
function create_function_proxy(sender, logger) {
  ...
  function exports(name, obj) {
    ...
    logger.debug('Exports module', name);
    ...
    for (var n in obj) {
      ...
      if (fn.constructor === Function) {
        ...
        logger.debug('Module', name, 'function', n);
      } else {
        mod.attr[n] = {
          name  : n,
          value : fn,
        };
        logger.debug('Module', name, 'attribute', n);
      }
    }
  }
  ...
  function peer_call_function(obj) {
      ...
      if (returned) {
        if (!err) {
          err = new Error('module ' + obj.modname
            + ' function ' + obj.fname + ' is return not repeat');
        }
  ...
  function recv(obj) {
    switch(obj.type) {
      ...
      // 接收到对函数的调用
      case 'call':
        peer_call_function(obj);
        break;

      // 接收到单条数据
      case 'message':
        callevent.emit('message', obj.name, obj.data);
        break;
